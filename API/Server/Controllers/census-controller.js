...
exports.getAllStates = function (req, res) {
  let createStateTable =
    "CREATE TABLE IF NOT EXISTS state(name VARCHAR(100),state int(255))";

  https
    .get(
      "https://api.census.gov/data/2019/pep/charagegroups?get=NAME&for=state:*&key=" +
        CENSUS_KEY,
      (resp) => {
        let data = "";
        ...
        resp.on("end", () => {
          ...
          let insertQ = "INSERT INTO state(name,state) values";
...
//Get Allstate counties
exports.getAllCounties = function (req, res) {
  ...
  https
    .get(
      "https://api.census.gov/data/2019/pep/charagegroups?get=NAME&for=county:*&key=" +
        CENSUS_KEY,
      (resp) => {
        let data = "";
        ...
        resp.on("end", () => {
          let censusData = JSON.parse(data);
          let insertQ = "INSERT INTO county(name,county) values";
          let selectQ = "SELECT * from county";
          let insertQ1 = "INSERT INTO county_state(state,county) values";
          let selectQ1 = "SELECT * from county_state";
          for (let i = 1; i < censusData.length; i++) {
            let ret = censusData[i][0].replace("'", "");
            ret = ret.split(",");
            insertQ =
          ...
          insertQ = insertQ.substring(0, insertQ.length - 1);
          insertQ1 = insertQ1.substring(0, insertQ1.length - 1);
          dbConfig.query(selectQ, function (err, result) {
            if (err) {
              dbConfig.query(createCountyTable, function (error, res_create) {
                if (error) {
                  ...
                  console.log("COUNTY TABLE CREATED");
                  dbConfig.query(insertQ, function (error_insert, response) {
                    if (error_insert) {
                      console.log(error_insert);
                      res.status(404).send({ success: false });
            ...
            if (err) {
              dbConfig.query(createCountyTable, function (error, res_create) {
                if (error) {
                  ...
                  console.log("COUNTY_STATE TABLE CREATED");
                  dbConfig.query(insertQ, function (error_insert, response) {
                    if (error_insert) {
                      console.log(error_insert);
                      res.status(404).send({ success: false });
